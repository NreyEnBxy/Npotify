...
export default function App() {
  ...
  const fallbackReels: Song[] = [
    { id: 'r1', title: 'Nature Vibes', artist: 'Earth', cover: 'https://picsum.photos/400', url: 'https://cdn.pixabay.com/video/2020/05/26/40286-425624733_large.mp4', isReel: true },
    { id: 'r2', title: 'City Lights', artist: 'Urban', cover: 'https://picsum.photos/401', url: 'https://cdn.pixabay.com/video/2021/08/11/84687-586861541_large.mp4', isReel: true },
    { id: 'r3', title: 'Ocean Waves', artist: 'Water', cover: 'https://picsum.photos/402', url: 'https://cdn.pixabay.com/video/2020/10/24/53127-474015694_large.mp4', isReel: true }
  ];

  const reelSongs = useMemo(() => {
    const reels = songs.filter(s => s.isReel);
    return reels.length > 0 ? reels : fallbackReels;
  }, [songs]);
  ...
  const playSong = (songParam: number | string | Song) => {
    if (typeof songParam === 'object') {
       // If it's an API song not in the current list, add it
       if (!songs.find(s => s.id === songParam.id)) {
          setSongs(prev => [...prev, songParam]);
       }
       setCurrentSongIndex(songParam.id);
    } else {
      if (currentSongIndex === songParam) {
        togglePlay();
      } else {
        setCurrentSongIndex(songParam);
      }
    }
  ...
  useEffect(() => {
    const fetchApiSearch = async () => {
      ...
      try {
         const res = await fetch(`/api/saavn/search?query=${encodeURIComponent(searchQuery)}`);
         if (res.ok) {
    ...
    const debounce = setTimeout(fetchApiSearch, 500);
    return () => clearTimeout(debounce);
  }, [searchQuery]);

  const filteredLocalSongs = songs.filter(song => 
    song.title.toLowerCase().includes(searchQuery.toLowerCase()) || 
    song.artist.toLowerCase().includes(searchQuery.toLowerCase())
  );
  
  // Dedup API songs from local songs to prevent duplicates
  const filteredSongs = [...filteredLocalSongs, ...apiSongs.filter(as => !filteredLocalSongs.some(ls => ls.title.toLowerCase() === as.title.toLowerCase()))];
